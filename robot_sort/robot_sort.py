class SortingRobot:
    ...
    def sort(self):
        ...
        #         self.move_left()
            ...
            # if self.can_move_left == false and self.can_move_right() == false:
        # to start 
        self.set_light_on()
        ...
        while self.light_is_on():
            ...
            while self.can_move_right():
                self.move_right()
            #go back to left and compare
            while self.can_move_left():
                self.swap_item()
                self.move_left()
                # then compare, if -1 it is less than, so swap
                if self.compare_item() == -1:
                    # repeat the same process moving right until finding the next smallest item in the list
                    self.swap_item()
                    #turn light on to keep going
                    self.set_light_on()
                self.move_right()
                self.swap_item()
                self.move_left()
